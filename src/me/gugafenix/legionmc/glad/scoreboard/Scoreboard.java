...
public class Scoreboard {
	...
	public Scoreboard(Player player, String objectiveName) {
		this.player = player;
		this.objectiveName = player.getName();
		ScoreManager.getManager().getScoreboards().add(this);
	}
	...
	public void destroy() {
		...
		for (VirtualTeam team : lines) if (team != null) getPlayer().sendPacket(team.removeTeam());
	...
	public void update() {
		...
		setObjectiveName(replace(getBukkitPlayer(), glad.getScoreboard().get(0)));
		int blank = 0;
		for (int i = 1; i < glad.getScoreboard().size(); i++) {
			String line = replace(getBukkitPlayer(), glad.getScoreboard().get(i));
			
			if (line == null) break;
			
			if (line.equalsIgnoreCase("%empty%")) blank++;
			String space = "";
			if (blank > 1) for (int j = 0; j < blank; j++) space += " ";
			
			setLine(i, replace(getBukkitPlayer(), line.replace("%empty%", space)));
		}
	}
	...
	private PlayerConnection getPlayer() { return ((CraftPlayer) player).getHandle().playerConnection; }
	...
	private void sendLine(int line) {
		...
		for (Packet packet : val.sendLine()) getPlayer().sendPacket(packet);
	...
	private PacketPlayOutScoreboardScore removeLine(String line) { return new PacketPlayOutScoreboardScore(line); }
	...
	public class VirtualTeam {
		...
		private VirtualTeam(String name) { this(name, "", ""); }
		...
		public PacketPlayOutScoreboardTeam createTeam() { return createPacket(0); }
		
		public PacketPlayOutScoreboardTeam updateTeam() { return createPacket(2); }
		...
		public Iterable<PacketPlayOutScoreboardTeam> sendLine() {
			...
			} else if (prefixChanged || suffixChanged) { packets.add(updateTeam()); }
		...
		public PacketPlayOutScoreboardTeam changePlayer() { return addOrRemovePlayer(3, currentPlayer); }
		...
		public String getValue() { return getPrefix() + getCurrentPlayer() + getSuffix(); }
		...
		public void setValue(String value) {
			if (value.length() <= 16) {
				...
				setPrefix(value.substring(0, 16));
				setPlayer(value.substring(16, 32));
				setSuffix(value.substring(32, 46));
			}
		}
	}
