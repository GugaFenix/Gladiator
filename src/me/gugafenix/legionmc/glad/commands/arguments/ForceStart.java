...
public class ForceStart {
	...
	public void execute(Player p, String cmd, String[] args) {
		...
		if (task == new Tasks(TaskId.START_BATTLE, glad).getStartBattle() || glad.getTaskRunning() == null) {
		...
		for (GladPlayer gp : glad.getPlayers()) {
			...
			gp.getPlayer().sendMessage(Main.tag + "§3O início do gladiado foi sancionado mais cedo por " + p.getName());
		}
